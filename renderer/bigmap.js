...
(function () {
  ...
  function baseChoice() {
    return snap.settings.mapBase === "re3mr" && mapPayload && mapPayload.re3mr ? "re3mr" : "tiles";
  }
  ...
  function ensureMap() {
    ...
    const sig = `${key}|${base}|${base === "tiles" ? style : "-"}|${snap.settings.extrudeDepth}`;
    ...
    $("map").innerHTML = "";
    const b = TM.buildMap($("map"), mapPayload, { base, style: base === "tiles" && style !== "none" ? style : null, extrudeDepth: snap.settings.extrudeDepth });
  ...
  function paintControls() {
    ...
    baseChip.className = "chip" + (snap.settings.mapBase === "re3mr" && hasR ? " on" : "") + (mapPayload.re3mrAvailable && !hasR ? " f" : "");
    baseChip.textContent = hasR ? "RE3MR 3D" : mapPayload.re3mrAvailable ? (snap.re3mrProgress && snap.re3mrProgress[mapPayload.def.key] ? "RE3MR 3D · " + snap.re3mrProgress[mapPayload.def.key].stage : "RE3MR 3D · not aligned") : "RE3MR 3D · n/a";
    baseChip.onclick = () => window.api.saveSettings({ mapBase: snap.settings.mapBase === "re3mr" ? "tiles" : "re3mr" });
    st.appendChild(baseChip);
    for (const [id, label] of STYLES) {
      const c = document.createElement("span");
      c.className = "chip" + (snap.settings.mapBase !== "re3mr" || !hasR ? (snap.settings.mapStyle === id ? " on" : "") : " dim");
      c.textContent = label;
      c.onclick = () => window.api.saveSettings({ mapBase: "tiles", mapStyle: id });
      st.appendChild(c);
    }
    ...
    $("notice").style.display = missing ? "" : "none";
  ...
  function paintMarkers() {
    ...
    const key = JSON.stringify([[...on()], snap.objectives.map((o) => o.objectiveId), snap.game.side, Boolean(mapPayload.features), snap.settings.fleaMin]);
